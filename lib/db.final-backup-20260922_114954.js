...
function getCloudPool() {
  ...
  if (!databaseUrl) {
    const availableKeys = Object.keys(process.env)
      .filter(key => /DATABASE|MYSQL|TIDB/i.test(key))
      .sort();

    throw new Error(
      "DATABASE_URL is not configured at runtime. " +
      "Database-related environment keys visible: " +
...
const cloudDb = {
  async all(sql, params = []) {
    const [rows] = await getCloudPool().execute(sql, params);
    return rows;
  },
  ...
  async run(sql, params = []) {
    const [result] = await getCloudPool().execute(sql, params);
